		...
		<main className="flex min-h-screen bg-gray-100 md:flex md:justify-center">
			<div className="w-full max-w-md bg-white rounded-lg shadow-lg  max-md:m-auto">
				...
				<section className="ml-4 my-8 min-h-[50vh] flex flex-col gap-1">
					...
					<input type="text" disabled placeholder="Documento" value={patientInfo?.numeroDocumento} className="p-3 border-2 border-solid border-gray-400 rounded-xl w-[95%]" />
					<div className="flex flex-row mt-4 items-center">
						<BloodIcon width={16} height={16} />
						<p className="ml-3 font-inter font-bold">Patologia</p>
					</div>
					<input type="text" disabled placeholder="Patologia" value={patientInfo?.patologia} className="p-3 border-2 border-solid border-gray-400 rounded-xl w-[95%]" />
					<div className="flex flex-row mt-4 items-center">
						<ShieldIcon width={16} height={16} />
						<p className="ml-3 font-inter font-bold">Medicos</p>
					</div>
					<input type="text" disabled placeholder="Medicos" value={patientInfo?.medicos[0]} className="p-3 border-2 border-solid border-gray-400 rounded-xl w-[95%]" />
					<div className="flex flex-row mt-4 items-center">
						<HospitalIcon width={20} height={20} />
						<p className="ml-3 font-inter font-bold">Hospital</p>
					</div>
					<input type="text" disabled placeholder="Medicos" value={patientInfo?.entidades[0]} className="p-3 border-2 border-solid border-gray-400 rounded-xl w-[95%]" />
				</section>
