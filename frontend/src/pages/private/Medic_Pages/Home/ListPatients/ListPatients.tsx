...
export default function ListPatients() {
  ...
  const filteredPatients = patients && patients.content
  ...
  const displayedPatients = searchQuery ? filteredPatients : filteredPatients.slice(0, 4);
  ...
  return (
    <>
      ...
      <div className="mb-4 flex items-center justify-between">
        <h2 className="font-bold  font-inter text-violet-color">Lista de pacientes</h2>
      </div>
          ...
          {loading ? (
            ...
            {patients && displayedPatients.length > 0 ? (
              ...
              <div className=" mt-32 flex items-center justify-center flex-col gap-2">
                <p className="text-center h-full text-gray-600 font-semibold">No hay pacientes disponibles</p>
                <Link to={"/patient-register"}>
                <button className="rounded-lg w-32 h-10 py-1 px-3 shadow-md border-2 border-violet-color  font-semibold hover:scale-105 transition-all duration-300 text-[14px]  border-solid text-violet-color">Crear paciente</button>
                </Link>
              </div>
            )}
          </>}
