...
export default function TreatmentAdherence() {
  ...
  const fetchTreatmentAdherence = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/adherencia/listar-adherencias-por-tratamiento/${idTratamiento}`, {
        method: "GET",
        headers: {
      ...
      setInfo([
        ...
        { name: 'No compleados', datos: data.totalNoCompletado, amt: data.totalNoCompletado },
        { name: 'Retrasados', datos: data.totalRetrasados, amt: data.totalRetrasados },
      ]);
    } catch (error: any) {
      console.log(error)
    }
    ...
    <main className="flex min-h-screen bg-gray-100 md:flex md:justify-center ">
      <div className="w-full max-w-md  min-h-screen pb-4  bg-white rounded-lg shadow-lg font-inter  max-md:m-auto">
        ...
        <section className='px-6 h-[120vh] '>
          <h1 className=' mb-4 text-violet-color font-bold text-lg'>Adherencia a la medicación</h1>
          <div className=' flex justify-centeri justify-around'>
            <h1 className=' gap-2 text-4xl flex flex-col justify-center items-center'>{adherence?.totalCompletado}<span className=' text-sm font-semibold'>Compleados</span> <span className='w-12 rounded-md h-1 bg-secondary-brand-dark'></span></h1>
            <h1 className=' gap-2 text-4xl flex flex-col justify-center items-center'>{adherence?.totalNoCompletado}<span className=' text-sm font-semibold'>No Compleados</span> <span className='w-12 rounded-md h-1 bg-blue-light-color'></span></h1>
            <h1 className=' gap-2 text-4xl flex flex-col justify-center items-center'>{adherence?.totalRetrasados}<span className=' text-sm font-semibold'>Retrasados</span> <span className='w-12 rounded-md h-1 bg-sucess-state'></span></h1>
          </div>
          <div style={{ width: '100%', height: 300 }} className='mt-4'>
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Bar dataKey="datos" fill="#8884d8" background={{ fill: '#eee' }} />
              </BarChart>
            </ResponsiveContainer>
            <ResponsiveContainer width="100%" height="100%">
